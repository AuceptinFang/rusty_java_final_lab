...
impl Seat{
    ...
    pub async fn book(&self, id : &str, app: AppHandle) -> Result<String> {
        let result = {
            ...
            if !guard.available_seats.is_empty(){
                ...
                guard.seats -= 1;
                let new_status = SeatJson{
                    name : seat_id.clone(),
                    state : true,
                };
                app.emit("seats-update",new_status)?;
                Ok(seat_id)
            }else{
                anyhow::bail!("无可用座位")
            }
    ...
    pub async fn cancel(&self, id: &str, app: AppHandle) -> Result<String> {
        let result = {
            let mut guard = self.state.lock();
