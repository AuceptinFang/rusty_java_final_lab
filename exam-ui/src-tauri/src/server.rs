...
#[tauri::command]
pub async fn server(app: AppHandle) -> Result<(),String> {
    let port = 5000;
    ...
    tauri::async_runtime::spawn(async move{
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = handle_conn(socket, seat_clone, app_handle).await {
                    error!("未预期的异常{}", e);
                }
            });
        }
...
async fn handle_conn(mut socket: TcpStream, seat: Arc<Seat>, app: AppHandle) -> anyhow::Result<()> {
    if let Ok(msg) = read_utf(&mut socket).await {
        match msg.find('|') {
            Some(index) => {
                ...
                let id = &msg[index + 1..];
                match option.as_str() {
                    "BOOK" => match seat.book(id, app).await {
                        Ok(seat_id) => {
                    ...
                    "CANCEL" => match seat.cancel(id,app).await {
                        Ok(seat_id) => {
                            write_utf(&mut socket, &format!("Ok|{}", seat_id)).await?;
                            Ok(())
                        }
